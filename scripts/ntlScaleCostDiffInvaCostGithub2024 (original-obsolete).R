...
# model set
vars <- c("gdp","cpi","ghsi","igs")    
vars4comb <- paste("ratio~", apply(combn(vars,4),2,paste,collapse='+'),"+(1|reg2)", sep="")
vars3comb <- paste("ratio~", apply(combn(vars,3),2,paste,collapse='+'),"+(1|reg2)", sep="")
vars2comb <- paste("ratio~", apply(combn(vars,2),2,paste,collapse='+'),"+(1|reg2)", sep="")
vars1comb <- paste("ratio~", vars, "+(1|reg2)", sep="")
interconly <- "ratio~1+(1|reg2)"
...
fitsat <- lmer(as.formula(mod.vec[1]), data=final.dat.imputed, na.action=na.omit)
...
plot_model(fitsat, type="std", sort.est=T)
plot_model(fitsat, type="re")
plot_model(fitsat, type="slope", sort.est=T)
plot_model(fitsat, type="resid", sort.est=T)
plot_model(fitsat, type="diag", grid=T)
...
brt.fitF <- gbm.step(final.dat.imputed, gbm.x = attr(final.dat.imputed, "names")[c(6,8,9,10)], gbm.y = attr(final.dat.imputed, "names")[4], family="gaussian", max.trees=100000, tolerance = 0.00001, learning.rate = 0.00003, bag.fraction=0.75, tree.complexity = 2)
summary(brt.fitF)
...
# create storage arrays
val.arr <- pred.arr <- array(data = NA, dim = c(eq.sp.points, 4, biter), dimnames=list(paste("x",1:eq.sp.points,sep=""), attr(final.dat.imputed, "names")[c(6,8,9,10)], paste("b",1:biter,sep="")))

# create storage vectors
D2.vec <- CV.cor.vec <- CV.cor.se.vec <- igs.ri <- cpi.ri <- VAag.ri <- agrL.ri <- rep(NA,biter)
...
for (b in 1:biter) {
  ...
  # boosted regression tree
  brt.fit <- gbm.step(dat.resamp, gbm.x = attr(dat.resamp, "names")[c(6,8,9,10)], gbm.y = attr(dat.resamp, "names")[4], family="gaussian", max.trees=100000, tolerance = 0.0001, learning.rate = 0.0001, bag.fraction=0.75, tree.complexity = 2, silent=T, tolerance.method = "auto")
  summ.fit <- summary(brt.fit)

  length(summ.fit[[1]])
  
  if (length(summ.fit[[1]]) == 4) {
    # variable relative importance
    cpi.ri[b] <- summ.fit$rel.inf[which(summ.fit$var == attr(dat.resamp, "names")[c(6,8,9,10)][1])]
    igs.ri[b] <- summ.fit$rel.inf[which(summ.fit$var == attr(dat.resamp, "names")[c(6,8,9,10)][2])]
    agrL.ri[b] <- summ.fit$rel.inf[which(summ.fit$var == attr(dat.resamp, "names")[c(6,8,9,10)][3])]
    VAag.ri[b] <- summ.fit$rel.inf[which(summ.fit$var == attr(dat.resamp, "names")[c(6,8,9,10)][4])]
...
par(mfrow=c(2,2)) 
plot(val.med[,1],pred.med[,2],type="l",ylim=c(min(pred.lo[,1]),max(pred.up[,1])), lwd=2, ylab="(←lower) dam:mgm (higher→)", xlab="(←higher) corruption (lower→)")
lines(val.med[,1], pred.lo[,2], type="l", lty=2, col="red")
lines(val.med[,1], pred.up[,2], type="l", lty=2, col="red")

plot(val.med[,2],pred.med[,3],type="l",ylim=c(min(pred.lo[,2]),max(pred.up[,2])), lwd=2, ylab="(←lower) dam:mgm (higher→)",  xlab="(←lower) per capita imports goods & services (higher→)" )
lines(val.med[,2], pred.lo[,3], type="l", lty=2, col="red")
lines(val.med[,2], pred.up[,3], type="l", lty=2, col="red")

plot(val.med[,3],pred.med[,4],type="l",ylim=c(min(pred.lo[,3]),max(pred.up[,3])), lwd=2, ylab="(←lower) dam:mgm (higher→)", xlab="(←less) agricultural land (more→)")
lines(val.med[,3], pred.lo[,4], type="l", lty=2, col="red")
lines(val.med[,3], pred.up[,4], type="l", lty=2, col="red")

plot(val.med[,4],pred.med[,4],type="l",ylim=c(min(pred.lo[,4]),max(pred.up[,4])), lwd=2, ylab="(←lower) dam:mgm (higher→)", xlab="(←less) value-added agric (more→)")
lines(val.med[,4], pred.lo[,4], type="l", lty=2, col="red")
lines(val.med[,4], pred.up[,4], type="l", lty=2, col="red")
par(mfrow=c(1,1)) 
...
ri.lo <- c(cpi.ri.lo,igs.ri.lo,agrL.ri.lo,VAag.ri.lo)
ri.med <- c(cpi.ri.med,igs.ri.med,agrL.ri.med,VAag.ri.med)
ri.up <- c(cpi.ri.up,igs.ri.up,agrL.ri.up,VAag.ri.up)

ri.out <- as.data.frame(cbind(ri.lo,ri.med,ri.up))
colnames(ri.out) <- c("ri.lo","ri.med","ri.up")
rownames(ri.out) <- attr(final.dat.imputed, "names")[c(6,8,9,10)]
ri.sort <- ri.out[order(ri.out[,2],decreasing=T),1:3]
ri.sort
...
vars <- c("gdp","igs","cpi","ghsi")    
vars4comb <- paste("pdam~", apply(combn(vars,4),2,paste,collapse='+'),"+(1|reg2)", sep="")
vars3comb <- paste("pdam~", apply(combn(vars,3),2,paste,collapse='+'),"+(1|reg2)", sep="")
vars2comb <- paste("pdam~", apply(combn(vars,2),2,paste,collapse='+'),"+(1|reg2)", sep="")
vars1comb <- paste("pdam~", vars, "+(1|reg2)", sep="")
interconly <- "pdam~1+(1|reg2)"
mod.vec <- c(vars4comb,vars3comb,vars2comb,vars1comb,interconly)
mod.vec  
...
# GLMM
# model set
vars <- c("gdp","agrL","stja","govexpedu")    
vars4comb <- paste("pdam~", apply(combn(vars,4),2,paste,collapse='+'),"+(1|reg2)", sep="")
vars3comb <- paste("pdam~", apply(combn(vars,3),2,paste,collapse='+'),"+(1|reg2)", sep="")
vars2comb <- paste("pdam~", apply(combn(vars,2),2,paste,collapse='+'),"+(1|reg2)", sep="")
vars1comb <- paste("pdam~", vars, "+(1|reg2)", sep="")
...
sumtableF2 <- data.frame(mod.num,k.vec,round(LL.vec,3),round(AICc.vec,3),round(dAICc,3),
                         round(wAICc,3),round(BIC.vec,3),round(dBIC,3),round(wBIC,4),round(Rm,1),round(Rc,1))
...
fitsat <- lmer(as.formula(mod.vec[1]), data=final.datPdam.imputed, na.action=na.omit)
...
# run with spheroid spatial autocorrelation
# model set
vars <- c("gdp","igs","cpi","VAag","agrL")    
vars5comb <- paste("ratio~", apply(combn(vars,5),2,paste,collapse='+'), sep="")
vars4comb <- paste("ratio~", apply(combn(vars,4),2,paste,collapse='+'), sep="")
vars3comb <- paste("ratio~", apply(combn(vars,3),2,paste,collapse='+'), sep="")
vars2comb <- paste("ratio~", apply(combn(vars,2),2,paste,collapse='+'), sep="")
vars1comb <- paste("ratio~", vars, sep="")
interconly <- "ratio~1"
...
for(i in 1:Modnum) {
  fit <- gls(as.formula(mod.vec[i]), correlation = corSpher(form = ~ lon + lat, nugget = T), method="ML", data = final.dat.imputed4)
  assign(paste("fit",i,sep=""), fit)
  mod.list[[i]] <- fit
  LL.vec[i] <- logLik(fit)
...
## phase-based approach ##
##########################
## response: dam:mgm rate of change (r)
head(final.datr.imputed)

## PHASE 1 - wealth/capacity
# gdp, cpi, ghsi, igs
brt.fit1 <- gbm.step(final.datr.imputed, gbm.x = attr(final.datr.imputed, "names")[c(5:8)],
                     gbm.y = attr(final.datr.imputed, "names")[4], family="gaussian",
                     max.trees=100000, tolerance = 0.0001, learning.rate = 0.0000001,
                     bag.fraction=0.7, tree.complexity = 2, step.size=0.0001)
summary(brt.fit1)
D2 <- 100 * (brt.fit1$cv.statistics$deviance.mean - brt.fit1$self.statistics$mean.resid) / brt.fit1$cv.statistics$deviance.mean
D2 # % deviance explained
gbm.plot(brt.fit1)
...
sumtable1 <- data.frame(mod.num,k.vec,round(LL.vec,3),round(AICc.vec,3),round(dAICc,3),
                        round(wAICc,3),BIC.vec,round(dBIC,3),round(wBIC,4),round(Rm,1),round(Rc,1))
colnames(sumtable1) <- c("model","k","LL","AICc","dAICc","wAICc","BIC","dBIC","wBIC","Rm","Rc")
row.names(sumtable1) <- as.character(mod.vec)
summary.table1 <- sumtable1[order(sumtable1[,5],decreasing=F),]
summary.table1

setwd("/Users/brad0317/Documents/Papers/Invasive species/Economics/InvaCost/national-scale extrapolation/out")
write.table(summary.table1, "glmm.r.sumtable.ph1.csv", sep=",", row.names = T)

fitsat <- lmer(as.formula(mod.vec[1]), data=final.datr.imputed, na.action=na.omit)
check_model(fitsat, detrend=F)
...
sumtable2 <- data.frame(mod.num,k.vec,round(LL.vec,3),round(AICc.vec,3),round(dAICc,3),
                        round(wAICc,3),BIC.vec,round(dBIC,3),round(wBIC,3),round(Rm,1),round(Rc,1))
colnames(sumtable2) <- c("model","k","LL","AICc","dAICc","wAICc","BIC","dBIC","wBIC","Rm","Rc")
row.names(sumtable2) <- as.character(mod.vec)
summary.table2 <- sumtable2[order(sumtable2[,5],decreasing=F),]
summary.table2
...
brt.fit3 <- gbm.step(final.datr.imputed, gbm.x = attr(final.datr.imputed, "names")[c(11:12)],
                     gbm.y = attr(final.datr.imputed, "names")[4], family="gaussian", max.trees=100000,
                     tolerance = 0.000001, learning.rate = 0.0000001, step.size=0.0001, bag.fraction=0.75,
                     tree.complexity = 2)
summary(brt.fit3)
D2 <- 100 * (brt.fit3$cv.statistics$deviance.mean - brt.fit3$self.statistics$mean.resid) / brt.fit3$cv.statistics$deviance.mean
D2 # % deviance explained
...
sumtable3 <- data.frame(mod.num,k.vec,round(LL.vec,3),round(AICc.vec,3),round(dAICc,3),
                        round(wAICc,3),BIC.vec,round(dBIC,3),round(wBIC,3),round(Rm,1),round(Rc,1))
colnames(sumtable3) <- c("model","k","LL","AICc","dAICc","wAICc","BIC","dBIC","wBIC","Rm","Rc")
row.names(sumtable3) <- as.character(mod.vec)
summary.table3 <- sumtable3[order(sumtable3[,5],decreasing=F),]
...
ran.eff <- "+(1|reg2)"
resp.var <- "r~"
vars1comb <- paste(resp.var, vars, ran.eff, sep="")
interconly <- paste(resp.var,1,ran.eff,sep="")
...
sumtableF <- data.frame(mod.num,k.vec,round(LL.vec,3),round(AICc.vec,3),round(dAICc,3),
                        round(wAICc,3),BIC.vec,round(dBIC,3),round(wBIC,3),round(Rm,1),round(Rc,1))
colnames(sumtableF) <- c("model","k","LL","AICc","dAICc","wAICc","BIC","dBIC","wBIC","Rm","Rc")
row.names(sumtableF) <- as.character(mod.vec)
summary.tableF <- sumtableF[order(sumtableF[,5],decreasing=F),]
...
brt.fitF <- gbm.step(final.datr.imputed, gbm.x = attr(final.datr.imputed, "names")[c(9)],
                     gbm.y = attr(final.datr.imputed, "names")[4], family="gaussian", max.trees=100000,
                     tolerance = 0.000001, learning.rate = 0.00002, bag.fraction=0.75, tree.complexity = 2)
summary(brt.fitF)
D2 <- 100 * (brt.fitF$cv.statistics$deviance.mean - brt.fitF$self.statistics$mean.resid) / 
  brt.fitF$cv.statistics$deviance.mean
D2 # % deviance explained
gbm.plot(brt.fitF)
gbm.plot.fits(brt.fitF)
...
# create storage arrays
valc.arr <- predc.arr <- array(data = NA, dim = c(eq.sp.points, 1, biter),
                               dimnames=list(paste("x",1:eq.sp.points,sep=""),
                                             attr(final.datr.imputed, "names")[c(9)], paste("b",1:biter,sep="")))

# create storage vectors
D2c.vec <- CVc.cor.vec <- CVc.cor.se.vec <- rep(NA,biter)

for (b in 1:biter) {
  # resample data among countries
  resamp.sub <- sort(sample(x = 1:dim(final.datr.imputed)[1], size = dim(final.datr.imputed)[1], replace=TRUE))
  dat.resamp <- final.datr.imputed[resamp.sub,]
  
  # boosted regression tree
  brt.fit <- gbm.step(dat.resamp, gbm.x = attr(dat.resamp, "names")[c(9)],
                      gbm.y = attr(dat.resamp, "names")[4], family="gaussian", max.trees=100000,
                      tolerance = 0.0001, learning.rate = 0.0001, bag.fraction=0.75,
                      tree.complexity = 2, silent=T, tolerance.method = "auto")
  summ.fit <- summary(brt.fit)
  length(summ.fit[[1]])
  
  as.numeric(summ.fit[1])==0
  
  if (exists("brt.fit")==T & as.character(summ.fit[1])=="agrL" & length(summ.fit[[1]]) == 1) {
    
    D2c.vec[b] <- 100 * (brt.fit$cv.statistics$deviance.mean - brt.fit$self.statistics$mean.resid) /
      brt.fit$cv.statistics$deviance.mean
    CVc.cor.vec[b] <- 100 * brt.fit$cv.statistics$correlation.mean
    CVc.cor.se.vec[b] <- 100 *brt.fit$cv.statistics$correlation.se
    
    RESPc.val <- RESPc.pred <- matrix(data=NA, nrow=eq.sp.points, ncol=1)
    ## output average predictions
    for (p in 1:1) {
      RESPc.val[,p] <- plot.gbm(brt.fit, i.var=p, continuous.resolution=eq.sp.points, return.grid=T)[,1]
      RESPc.pred[,p] <- plot.gbm(brt.fit, i.var=p, continuous.resolution=eq.sp.points, return.grid=T)[,2]
    }
    RESPc.val.dat <- as.data.frame(RESPc.val)
    colnames(RESPc.val.dat) <- brt.fit$var.names
    RESPc.pred.dat <- as.data.frame(RESPc.pred)
    colnames(RESPc.pred.dat) <- brt.fit$var.names
    
    valc.arr[, , b] <- as.matrix(RESPc.val.dat)
    predc.arr[, , b] <- as.matrix(RESPc.pred.dat)
    
    print(b)
  }
  
  if (exists("brt.fit")==F | as.character(summ.fit[1])!="agrL" | length(summ.fit[[1]]) != 1) {
    b <- b+1
    print(b)
  }
...
kappa <- 2
kappa.n <- 5
predc.update <- predc.arr[,,1:biter]

for (k in 1:kappa.n) {
  bootc.mean <- apply(predc.update, MARGIN=c(1), mean, na.rm=T)
  bootc.sd <- apply(predc.update, MARGIN=c(1), sd, na.rm=T)
  
  for (z in 1:biter) {
    predc.update[,z] <- ifelse((predc.update[,z] < (bootc.mean-kappa*bootc.sd) | 
                                  predc.update[,z] > (bootc.mean+kappa*bootc.sd)), NA, predc.update[,z])
  }
  print(k)
}

predc.med <- apply(predc.update, MARGIN=c(1), median, na.rm=T)
predc.lo <- apply(predc.update, MARGIN=c(1), quantile, probs=0.025, na.rm=T)
predc.up <- apply(predc.update, MARGIN=c(1), quantile, probs=0.975, na.rm=T)

valc.med <- apply(valc.arr[,,1:biter], MARGIN=c(1), median, na.rm=T)


# kappa method for output vectors
D2c.update <- D2c.vec[1:biter]
CVc.cor.update <- CVc.cor.vec[1:biter]
CVc.cor.se.update <- CVc.cor.se.vec[1:biter]

for (k in 1:kappa.n) {
  D2c.mean <- mean(D2c.update, na.rm=T); D2c.sd <- sd(D2c.update, na.rm=T)
  CVc.cor.mean <- mean(CVc.cor.update, na.rm=T); CVc.cor.sd <- sd(CVc.cor.update, na.rm=T)
  CVc.cor.se.mean <- mean(CVc.cor.se.update, na.rm=T); CVc.cor.se.sd <- sd(CVc.cor.se.update, na.rm=T)
  
  for (u in 1:biter) {
    D2c.update[u] <- ifelse((D2c.update[u] < (D2c.mean-kappa*D2c.sd) |
                               D2c.update[u] > (D2c.mean+kappa*D2c.sd)), NA, D2c.update[u])
    CVc.cor.update[u] <- ifelse((CVc.cor.update[u] < (CVc.cor.mean-kappa*CVc.cor.sd) 
                                 | CVc.cor.update[u] > (CVc.cor.mean+kappa*CVc.cor.sd)), NA, CVc.cor.update[u])
    CVc.cor.se.update[u] <- ifelse((CVc.cor.se.update[u] < (CVc.cor.se.mean-kappa*CVc.cor.se.sd)
                                    | CVc.cor.se.update[u] > (CVc.cor.se.mean+kappa*CVc.cor.se.sd)),
                                   NA, CVc.cor.se.update[u])
  }
...
D2c.med <- median(D2c.update, na.rm=TRUE)
D2c.lo <- quantile(D2c.update, probs=0.025, na.rm=TRUE)
D2c.up <- quantile(D2c.update, probs=0.975, na.rm=TRUE)
print(c(D2c.lo,D2c.med,D2c.up))

CVc.cor.med <- median(CVc.cor.update, na.rm=TRUE)
CVc.cor.lo <- quantile(CVc.cor.update, probs=0.025, na.rm=TRUE)
CVc.cor.up <- quantile(CVc.cor.update, probs=0.975, na.rm=TRUE)
print(c(CVc.cor.lo,CVc.cor.med,CVc.cor.up))
